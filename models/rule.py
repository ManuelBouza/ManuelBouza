    ...
    def onchange_method(self):
        ...
        if self.fields_type == 'many2many':
            ...
            for record in records:
                transient = self.env['many2many.value'].create(
                    {
                        'name': record.name,
                        'model_id': record.id,
                    }
            ...
            for record in records:
                transient = self.env['many2one.value'].create(
                    {
                        'name': record.name,
                        'model_id': record.id,
                    }
            ...
            return {'domain': {'many2one_value': [('id', 'in', transient_ids)]}}
            ...
            if not self.rule_field.related:
                ...
                for value in selection_value:
                    ...
                    transient = self.env['selection.value'].create(
                        {
                            'name': name,
                            'key_id': key_id,
                        }
            ...
            return {'domain': {'selection_value': [('id', 'in', transient_ids)]}}
...
def create_rule(self):
    ...
    rule = {
        'generator_id': self.rules_generator_id,
...
def edit(self):
    context = {
        ...
        'default_rules_generator_id': self.rules_generator.id,
